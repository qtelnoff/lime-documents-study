...
def get_segmentation_grid(height, width, num_rows, num_cols):
...
def main(path_to_data_info: Path, num_rows: int = 4, num_cols: int = 4):
    # init the results
    path_to_result = Path(results_dir) / "segmentation" / f"grid_{num_rows}x{num_cols}_without_bboxes"
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing images"):
        ...
        mask_background_without_ocr = get_segmentation_grid(h, w, num_rows=4, num_cols=4)

        selection = mask_ocr > 0
        mask_background_without_ocr[selection] = 0  # set the background segments to 0 where OCR segments are present

        # save the segmentation mask
        path_to_data_result = path_to_result / image_hash
        path_to_data_result.mkdir(parents=True, exist_ok=True)
        torch.save(mask_background_without_ocr, path_to_data_result / "segmentation_mask.pt")
